...
public class DownstreamBridge extends PacketHandler {
	...
	public void handle(KeepAlive alive) throws Exception {
		...
		if (timeout <= 0 || server.getKeepAlives().size() < timeout / 50) // Some people disable timeout, otherwise
																			// allow a theoretical maximum of 1
																			// keepalive per tick
		{
	...
	public void handle(ScoreboardObjective objective) throws Exception {
		...
		case 0:
			serverScoreboard.addObjective(
					new Objective(objective.getName(), objective.getValue(), objective.getType().toString()));
			break;
		case 1:
	...
	public void handle(Commands commands) throws Exception {
		...
		for (Map.Entry<String, Command> command : bungee.getPluginManager().getCommands()) {
			if (!bungee.getDisabledCommands().contains(command.getKey())
					&& commands.getRoot().getChild(command.getKey()) == null && command.getValue().hasPermission(con)) {
