...
public final class ChatComponentTransformer
{
    ...
    public BaseComponent[] transform(final ProxiedPlayer player, final boolean transformHover, BaseComponent... components) {
        ...
        for (final BaseComponent root : components) {
            if (root.getExtra() != null && !root.getExtra().isEmpty()) {
                final List<BaseComponent> list = Lists.newArrayList(this.transform(player, transformHover, (BaseComponent[])root.getExtra().toArray(new BaseComponent[root.getExtra().size()])));
                root.setExtra(list);
            }
    ...
    private void transformScoreComponent(final ProxiedPlayer player, final ScoreComponent component) {
        Preconditions.checkArgument(!this.isSelectorPattern(component.getName()), (Object)"Cannot transform entity selector patterns");
